...
# Add pipelines/ to path so data.load_dataset is importable
sys.path.insert(0, os.path.join(os.path.dirname(__file__), ".."))

from data.load_dataset import load_dataset_split, load_dataset

from config import Config
from model_adapters.factory import construct_model_base
from utils.hook_utils import get_activation_addition_input_pre_hook, get_all_direction_ablation_hooks

from submodules.generate_directions import generate_directions
from submodules.select_direction import select_direction, get_refusal_scores
from submodules.evaluate_jailbreak import evaluate_jailbreak
from submodules.evaluate_loss import evaluate_loss
...
def select_and_save_direction(cfg, model_base, harmful_val, harmless_val, candidate_directions):
    ...
    batch_size = getattr(model_base, 'pipeline_batch_size', 32)
    kl_threshold = getattr(model_base, 'kl_threshold', 0.1)
